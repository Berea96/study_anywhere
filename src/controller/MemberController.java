...
import ajax.MemberDeleteAjax;
import ajax.GetMemberRoomsAjax;
import ajax.ModifyEventAjax;
import ajax.RemoveEventAjax;
import ajax.deleteMemberRoomAjax;
import ajax.tempPassCheckAjax;
import ajax.updateMemberRoomsAjax;
...
public class MemberController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (command.equals("/memberJoin.do")) {
			...
			ajax = new MemberDeleteAjax();
			try {
				responseText = ajax.getJSON(request, response);
				response.getWriter().write(responseText);
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else if(command.equals("/getMemberRooms.do")) { 
			ajax = new GetMemberRoomsAjax();
			try {
				responseText = ajax.getJSON(request, response);
				response.getWriter().write(responseText);
			} catch (Exception e) {
		...
		} else if(command.equals("/updateMemberRoom.do")) { 
			ajax = new updateMemberRoomsAjax();
			try {
				responseText = ajax.getJSON(request, response);
				response.getWriter().write(responseText);
			} catch (Exception e) {
		...
		} else if(command.equals("/deleteMemberRoom.do")) { 
			ajax = new deleteMemberRoomAjax();
			try {
				responseText = ajax.getJSON(request, response);
				response.getWriter().write(responseText);
			} catch (Exception e) {
